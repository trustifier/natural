...
var predicates = {
    ...
    "CURRENT-WORD-HAS-TAG": current_word_has_tag,
    ...
    "CURRENT-WORD-POS-IS": current_word_pos_is,
 };

function current_word_pos_is(tagged_sentence, i, parameter) {
    return  (i === 0 && parameter.toUpperCase() === 'START')  ||
            (i === tagged_sentence.length - 1 && parameter.toUpperCase() === 'END') ||
            (i === Number(parameter));
}

function current_word_has_tag(tagged_sentence, i, parameter) {
    if(tagged_sentence[i].length < 3 ) {
        return tagged_sentence[i][1].toUpperCase() === parameter.toUpperCase();
    }
    return (tagged_sentence[i].slice(2).indexOf(parameter) >= 0);
}
// ==================================
